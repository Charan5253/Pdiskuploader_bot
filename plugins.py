...
async def search_user_in_community(client, message):
    try:
        await client.get_chat_member('@animatedmovie12', message.chat.id)
        await client.get_chat_member('@Movies_Bazar0371', message.chat.id)
    except UserNotParticipant:
        await message.reply_text(not_joined_community, parse_mode = 'html',reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Join our Channel.',url = 'https://t.me/animatedmovie12')],
        [InlineKeyboardButton('Join our Group.',url = 'https://t.me/Movies_Bazar0371')]
        ]))
        return
